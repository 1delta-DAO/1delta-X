...
| Module | Exports |
|---|---|
| `proto/codec` | `encode/decodeOrderAnnounce`, `…SoftCancel`, `…FillNotice`, `…OrderList`, `…StreamMessage`, `orderToProto`/`protoToOrder` — all `uint256`→`bytes`, `address`→20 bytes. |
| `messages` | `OrderAnnounce`, `OrderSoftCancel`, `FillNotice` (viem-typed). |
...
| `book` | `Book` — backfill → subscribe → verify → keyed map, with expiry, signed soft-cancel eviction, and periodic on-chain re-check. |
