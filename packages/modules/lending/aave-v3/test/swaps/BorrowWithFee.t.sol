...
///
///   items    = [ TAKE AaveV3CreditModule: borrow USDC → settlement ]
...
contract BorrowWithFeeTest is AaveModulesBase {
    ...
    function _approveMakerBorrowSide(uint256 borrowAmount) internal {
        bytes32 borrowRef = keccak256(abi.encode(OP_BORROW, AAVE_POOL, USDC, uint256(2)));
        vm.startPrank(maker);
        ...
        permit3.approveTaker(address(settlement), address(creditModule), borrowRef, uint160(borrowAmount), 0);
    ...
    function test_borrow_withSourcingFee_aaveV3() public {
        ...
        (uint160 remaining,) =
            permit3.takerAllowance(maker, address(settlement), address(creditModule), keccak256(abi.encode(OP_BORROW, AAVE_POOL, USDC, uint256(2))));
