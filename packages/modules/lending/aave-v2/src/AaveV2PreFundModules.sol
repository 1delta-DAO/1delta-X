...
// ──────────────── Aave v2 PRE-FUNDED one-sided modules ────────────────
//
...
// and this module deposits/repays it from its own balance. The maker's only
...
// received asset needs nothing — it never transits the maker's wallet. These are
// the v2 siblings of `AaveV3PreFundDepositModule` / `AaveV3PreFundRepayModule`; the
// only venue deviation is the entry point name (`pool.deposit`, v2's supply).
//
...
/// @dev    Replaces {AaveV2PreFundDepositModule}, {AaveV2PreFundRepayModule}. `Op` rides in
...
contract AaveV2PreFundModule is PreFundModuleBase, IMakerModule, IFundingSource {
    enum Op {
        Deposit,
        Repay
    }
    ...
    function makeOnBehalf(address onBehalfOf, uint256 forAmount, bytes calldata data) external override {
        ...
        IAaveV2Pool(pool).deposit(asset, forAmount, onBehalfOf, 0);
        SafeTransferLib.forceApprove(asset, pool, 0);
        } else if (op == uint256(Op.Repay)) {
        // Its own frame, matching the v3 sibling: an optimizer-less compile of the
        // five-field decode plus the repay call overflows this function's stack.
