...
contract LiquityV2LeverageTest is CoreSettlementBase {
    ...
    function setUp() public virtual override {
        ...
        addCollModule = new LiquityV2AddCollModule(address(permit3), address(settlement), COLLATERAL_REGISTRY);
    ...
    function _borrowData(uint256 maxUpfrontFee) internal view returns (bytes memory) {
        return abi.encode(uint8(0), BRANCH, troveId, BOLD, maxUpfrontFee, uint256(1000e18));
    }

    function _addCollData() internal view returns (bytes memory) {
        return abi.encode(BRANCH, troveId, WETH);
    }
