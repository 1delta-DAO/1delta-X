import { Book, decodeOrderAnnounce, decodeOrderSoftCancel, encodeOrderAnnounce, encodeOrderList, encodeStreamMessage, InMemoryTransport, StreamKind, topicsFor, Verifier, } from "@1delta-x/orderbook";
...
export async function buildServer(opts) {
    ...
    app.post("/orders", async (request, reply) => {
        ...
        if (!res.ok)
            return reply.code(422).send({ error: res.reason ?? "rejected", orderHash: res.orderHash });
    ...
    app.get("/orders", async (request, reply) => {
        ...
        if (q.maker)
            items = items.filter((a) => a.order.maker.toLowerCase() === q.maker.toLowerCase());
        if (q.tokenIn)
            items = items.filter((a) => a.order.legsIn.some((l) => l.token.toLowerCase() === q.tokenIn.toLowerCase()));
        if (q.tokenOut)
            items = items.filter((a) => a.order.legsOut.some((l) => l.token.toLowerCase() === q.tokenOut.toLowerCase()));
        if (q.side != null)
            items = items.filter((a) => String(a.order.side) === q.side);
